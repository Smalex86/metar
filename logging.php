...
function setLogMsg($msgStatus, $msg, $logFileName = '') {
	...
	if ($logFile = fopen(dirname(__FILE__).'/logs/' . $logFileName, (dirname(__FILE__).'/logs/' . $logFileName) ? 'ab' : 'wb'))
		fwrite($logFile, sprintf('%s :: %s :: %s'.PHP_EOL, date("Y-m-d H-i-s"), $msgStatusWord, $msg));
	fclose($logFile);
	return true;
};
